...
def signup(request):
	if request.method=="POST":
		if request.POST['password']==request.POST['cpassword']:
			...
			# account_sid = 'AC952b84bcfecc1afb7d32248fb8990218'
			# auth_token = '6fe9b3fddb5d2f1aa0f250555df7e2b7'
			# client = Client(account_sid, auth_token)
			# message = client.messages.create(body='OTP FOR SINH UP'+str(otp),from_='+18484209011',to='+91'+request.POST['mobile'])
...
def seller_edit_product(request,pk):
	...
	if request.method=="POST":
		...
		try:
			...
			pass
		product.save()
		msg="Product Updeted Successfully"
...
def search(request):
	try:
		...
		products=Product()
		user=User.objects.get(email=request.session['email'])
		products=Product.objects.filter(product_category__contains=search)
		return render(request,'index.html',{'products':products})
	except:
		products=Product.objects.filter(product_category__contains=search)
		return render(request,'index.html',{'products':products})
...
def order(request):
	user=User.objects.get(email=request.session['email'])
	carts=Cart.objects.filter(user=user,status=False)
	cartsdone=Cart.objects.filter(user=user,status=True)
	request.session['cart_count']=len(carts)
	return render(request,'order.html',{'carts':carts,'cartsdone':cartsdone})
